# Decide For Me — Build 02.3.3.1

## Authentication confirmation repair
Build 02.3.3.1 hardens the Supabase email-confirmation flow for the hosted Cloudflare deployment. Signup now supplies an explicit redirect back to the the Cloudflare development application URL, authentication callback errors are handled in-app, confirmed sessions ensure the server-side profile exists, and users can request a new confirmation email.
...
The frontend passes `emailRedirectTo` explicitly as `https://development-decide-for-me.tom-ellery.workers.dev/` for this development build, preventing localhost confirmation links during QA.
