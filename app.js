...
async function loadProfileV21() {
  ...
  window.currentProfile = data || { id: user.id, username: "", avatar_url: null };
...
function renderProfileStatsV21() {
  const rows = typeof getHistory === "function" ? (getHistory() || []) : [];
...
function renderHistoryV21() {
  const source = typeof getHistory === "function" ? (getHistory() || []) : [];
  const filtered = profileHistoryFilter === "all"
    ? source
    : source.filter(r => String(r.status || r.result || "").toLowerCase() === profileHistoryFilter);
  ...
  list.innerHTML = filtered.map(r => {
    const text = r.mission_text || r.mission || r.challenge_text || "Mission";
    const status = String(r.status || r.result || "").toUpperCase();
    return `<div class="history-item"><div>${escapeHtml(text)}</div><small>${escapeHtml(status)}</small></div>`;
  }).join("");
}
