...
async function finishMission() {
  ...
  const baseXP = DIFF[state.difficulty]?.xp || 0;
  ...
  try {
    ...
    showWarning(
      ...
      "Your Mission and XP were saved, but the public proof could not be posted. You can try again from the completion screen."
    );
    go("complete");
  }
...
async function createWallPost() {
  ...
  if (state.proof.type === "text") {
    proofContent = { text: String(state.proof.content || "") };
  } else if (state.proof.type === "photo" || state.proof.type === "video") {
  ...
  const row = {
    user_id: currentUser.id,
    mission_id: String(state.mission.id),
    category: state.category.key,
    difficulty: state.difficulty === "mission" ? "challenge" : state.difficulty,
    mission_text: state.mission.text,
  ...
  return `
    <div class="history-row">
      ...
      <button
        ...
        aria-label="Remove ${esc(row.challenge_name || "Mission")} from my history"
        onclick="removeHistoryItem('${esc(row.id || "")}')"
      >
        ×
      </button>
...
async function removeHistoryItem(historyId) {
  if (!currentUser?.id || !supabaseClient || !historyId) return;

  const confirmed = window.confirm(
    "Remove this Mission from your history?\n\nYour Mission statistics will stay unchanged, but any XP earned from this completed Mission will be deducted from your total XP."
  );

  if (!confirmed) return;

  try {
    const { data, error } = await supabaseClient.rpc("remove_history_item", {
      p_history_id: historyId
    });

    if (error) throw error;

    await initProfileV21();
    updateProgress();

  } catch (error) {
    console.error("Could not remove Mission from history:", error);
    showWarning(
      "COULD NOT REMOVE MISSION",
      error?.message || "Your Mission history could not be updated. Please try again."
    );
  }
}
