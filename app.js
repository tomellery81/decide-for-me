...
function nextMissionId(category) {
  ...
  const max = db().reduce((highest, mission) => {
    const match = /^c(\d+)$/i.exec(String(mission.id || ""));
...
document.addEventListener("DOMContentLoaded", () => {
  setTimeout(() => {
    if (typeof initProfileV21 === "function") initProfileV21();
  }, 0);
});
