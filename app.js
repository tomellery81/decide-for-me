...
async function finishMission() {
  if (!state.mission || !currentUser?.id || !supabaseClient) return;
...
async function removeHistoryItem(historyId) {
  ...
  const confirmed = window.confirm(
    "Remove this Mission from your history?\n\nThe Mission itself will not be deleted. Your completion record will be retained securely as an audit record."
  );
  ...
  try {
    ...
    showWarning(
      ...
      "Your Mission history could not be updated. Please try again."
    );
  }
}
...
async function loadProfileV21() {
  ...
  if (historyResult.error) {
    ...
    profileV21History = historyResult.data || [];
  }
