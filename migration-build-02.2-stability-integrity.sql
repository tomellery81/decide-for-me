...
-- ============================================================
-- BUILD 02.3.1 — DAILY LIMIT TOGGLE + WALL MODERATION READINESS
-- ============================================================
-- Recreate the attempt-limit RPCs explicitly in the release migration.
-- Earlier builds defined these only in the baseline schema, which meant an
-- existing hosted database could retain an older/missing implementation.
alter table public.app_settings
  add column if not exists updated_by uuid references auth.users(id) on delete set null;
insert into public.app_settings(key, value, updated_at)
values ('daily_attempt_limit_mode', 'limited', now())
...
declare
  v_mode text;
begin
  select lower(trim(value)) into v_mode
...
begin
  ...
  if v_mode not in ('limited','unlimited') then
    raise exception 'Invalid attempt limit mode';
  end if;
...
-- ============================================================
-- BUILD 02.3.2 — SECURITY AND LIFECYCLE REPAIR
-- ============================================================
-- Existing 02.2 installations may not have this audit column even though
-- the 02.3.1 toggle function writes it.
alter table public.app_settings
  add column if not exists updated_by uuid references auth.users(id) on delete set null;

-- A daily attempt is an acceptance event, not a row that happens to still
-- be active.  The index also supports the scheduled global expiry sweep.
create index if not exists idx_history_user_daily_attempts
  on public.challenge_history(user_id, created_at)
  where status in ('accepted','completed','failed');
create index if not exists idx_history_expiry_due
  on public.challenge_history(expires_at)
  where status='accepted' and deleted_at is null;

create or replace function public.get_daily_attempt_status()
returns jsonb language plpgsql security definer stable set search_path=public as $$
declare v_used integer; v_mode text:='limited'; v_limit integer:=10; v_now timestamptz:=now();
begin
  ...
  if v_mode='unlimited' then v_limit:=null; end if;
  select count(*)::integer into v_used from public.challenge_history
  where user_id=auth.uid() and status in ('accepted','completed','failed')
    and created_at >= date_trunc('day',v_now at time zone 'UTC') at time zone 'UTC'
    and created_at < (date_trunc('day',v_now at time zone 'UTC')+interval '1 day') at time zone 'UTC';
  return jsonb_build_object('used',v_used,'remaining',case when v_mode='unlimited' then null else greatest(v_limit-v_used,0) end,'limit',v_limit,'allowed',v_mode='unlimited' or v_used<v_limit,'mode',v_mode);
end; $$;
...
declare
  v_user_id uuid:=auth.uid(); v_used integer; v_limit integer:=10; v_mode text:='limited';
  v_id uuid:=gen_random_uuid(); v_now timestamptz:=now(); v_expires timestamptz:=v_now+interval '24 hours';
  v_mission public.missions%rowtype; v_profile public.profiles%rowtype; v_active public.challenge_history%rowtype;
  v_position integer; v_number text; v_label text; v_forced integer;
begin
  if v_user_id is null then raise exception 'Authentication required'; end if;
  perform pg_advisory_xact_lock(hashtext(v_user_id::text)); perform public.expire_overdue_missions();
  select * into v_mission from public.missions where id=p_challenge_id and active=true;
  if not found then raise exception 'Mission not found or inactive'; end if;
  select * into v_active from public.challenge_history where user_id=v_user_id and status='accepted' and deleted_at is null order by created_at desc limit 1 for update;
  if found then return jsonb_build_object('allowed',false,'reason','active_mission','history_id',v_active.id,'expires_at',v_active.expires_at); end if;
  insert into public.profiles(id) values(v_user_id) on conflict(id) do nothing;
  select * into v_profile from public.profiles where id=v_user_id for update; v_forced:=coalesce(v_profile.forced_acceptances,0);
  select value into v_mode from public.app_settings where key='daily_attempt_limit_mode';
  if v_mode not in ('limited','unlimited') then v_mode:='limited'; end if; if v_mode='unlimited' then v_limit:=null; end if;
  select count(*)::integer into v_used from public.challenge_history where user_id=v_user_id and status in ('accepted','completed','failed') and created_at >= date_trunc('day',v_now at time zone 'UTC') at time zone 'UTC' and created_at < (date_trunc('day',v_now at time zone 'UTC')+interval '1 day') at time zone 'UTC';
  if v_mode<>'unlimited' and v_used>=v_limit then return jsonb_build_object('allowed',false,'used',v_used,'remaining',0,'limit',v_limit,'mode',v_mode); end if;
  v_position:=((nullif(regexp_replace(v_mission.id,'\D','','g'),'')::integer-1)%200)+1;
  v_number:=case v_mission.category when 'relationships' then '10' when 'finance' then '20' when 'work' then '30' when 'entertainment' then '40' when 'life-admin' then '50' when 'chores' then '60' else '00' end||'.'||lpad(v_position::text,6,'0');
  v_label:=initcap(replace(v_mission.category,'-',' '));
  insert into public.challenge_history(id,user_id,challenge_id,challenge_number,challenge_name,category,category_label,difficulty,status,created_at,accepted_at,expires_at) values(v_id,v_user_id,v_mission.id,v_number,v_mission.text,v_mission.category,v_label,v_mission.difficulty,'accepted',v_now,v_now,v_expires);
  if v_forced>0 then update public.profiles set forced_acceptances=greatest(v_forced-1,0),refusal_count=case when v_forced=1 then 0 else refusal_count end,updated_at=v_now where id=v_user_id; end if;
  v_used:=v_used+1;
  return jsonb_build_object('allowed',true,'used',v_used,'remaining',case when v_mode='unlimited' then null else greatest(v_limit-v_used,0) end,'limit',v_limit,'mode',v_mode,'history_id',v_id,'accepted_at',v_now,'expires_at',v_expires);
end; $$;

alter table public.challenge_history add column if not exists proof_type text;
alter table public.challenge_history add column if not exists proof_bucket text;
alter table public.challenge_history add column if not exists proof_path text;

insert into storage.buckets(id,name,public) values('private-proofs','private-proofs',false) on conflict(id) do update set public=false;
drop policy if exists "private proofs read own" on storage.objects;
drop policy if exists "private proofs upload own folder" on storage.objects;
drop policy if exists "private proofs delete own folder" on storage.objects;
create policy "private proofs read own" on storage.objects for select to authenticated using(bucket_id='private-proofs' and (storage.foldername(name))[1]=auth.uid()::text);
create policy "private proofs upload own folder" on storage.objects for insert to authenticated with check(bucket_id='private-proofs' and (storage.foldername(name))[1]=auth.uid()::text);
create policy "private proofs delete own folder" on storage.objects for delete to authenticated using(bucket_id='private-proofs' and (storage.foldername(name))[1]=auth.uid()::text);

drop function if exists public.complete_mission(uuid,text,integer);
create or replace function public.complete_mission(p_history_id uuid,p_proof_type text,p_bonus_xp integer,p_proof_bucket text default null,p_proof_path text default null)
returns jsonb language plpgsql security definer set search_path=public as $$
declare v_user_id uuid:=auth.uid(); v_now timestamptz:=now(); v_today date:=(v_now at time zone 'UTC')::date; v_history public.challenge_history%rowtype; v_profile public.profiles%rowtype; v_base integer; v_bonus integer; v_total integer; v_streak integer;
begin
  if v_user_id is null then raise exception 'Authentication required'; end if;
  if p_proof_type not in ('text','photo','video') then raise exception 'Invalid proof type'; end if;
  v_bonus:=case p_proof_type when 'text' then 0 when 'photo' then 25 when 'video' then 50 end;
  if p_bonus_xp is distinct from v_bonus then raise exception 'Invalid proof bonus'; end if;
  if p_proof_type in ('photo','video') then
    if p_proof_bucket not in ('public-proofs','private-proofs') or p_proof_path is null or p_proof_path !~ ('^'||v_user_id::text||'/') then raise exception 'A valid uploaded proof is required for media bonus XP'; end if;
    if not exists(select 1 from storage.objects where bucket_id=p_proof_bucket and name=p_proof_path) then raise exception 'Proof upload was not found'; end if;
  elsif p_proof_bucket is not null or p_proof_path is not null then raise exception 'Text proof cannot include media metadata'; end if;
  select * into v_history from public.challenge_history where id=p_history_id and user_id=v_user_id for update;
  if not found or v_history.status<>'accepted' or v_history.deleted_at is not null then raise exception 'This Mission has already been resolved or could not be found'; end if;
  if v_history.expires_at is not null and v_history.expires_at<=v_now then update public.challenge_history set status='failed',failed_at=v_now where id=p_history_id; return jsonb_build_object('ok',false,'status','expired','history_id',p_history_id); end if;
  v_base:=case v_history.difficulty when 'easy' then 25 when 'normal' then 50 when 'challenge' then 75 when 'brutal' then 100 when 'wild' then 150 else null end; if v_base is null then raise exception 'Invalid Mission difficulty'; end if; v_total:=v_base+v_bonus;
  select * into v_profile from public.profiles where id=v_user_id for update; if not found then insert into public.profiles(id) values(v_user_id) returning * into v_profile; end if;
  if v_profile.last_complete_date=v_today then v_streak:=coalesce(v_profile.streak,0); elsif v_profile.last_complete_date=v_today-1 then v_streak:=coalesce(v_profile.streak,0)+1; else v_streak:=1; end if;
  update public.challenge_history set status='completed',xp_earned=v_total,completed_at=v_now,proof_type=p_proof_type,proof_bucket=p_proof_bucket,proof_path=p_proof_path where id=p_history_id;
  update public.profiles set xp=coalesce(v_profile.xp,0)+v_total,streak=v_streak,last_complete_date=v_today,updated_at=v_now where id=v_user_id;
  return jsonb_build_object('ok',true,'history_id',p_history_id,'xp_earned',v_total,'xp',coalesce(v_profile.xp,0)+v_total,'streak',v_streak);
end; $$;
grant execute on function public.complete_mission(uuid,text,integer,text,text) to authenticated;

revoke insert on public.wall_posts from authenticated;
drop policy if exists "wall create own" on public.wall_posts;
create or replace function public.create_wall_post(p_history_id uuid,p_proof_text text default null,p_proof_path text default null)
returns jsonb language plpgsql security definer set search_path=public as $$
declare v_user_id uuid:=auth.uid(); v_history public.challenge_history%rowtype; v_post public.wall_posts%rowtype; v_content jsonb;
begin
  if v_user_id is null then raise exception 'Authentication required'; end if;
  select * into v_history from public.challenge_history where id=p_history_id and user_id=v_user_id for update;
  if not found or v_history.status<>'completed' or v_history.deleted_at is not null then raise exception 'Only your completed Missions can be posted'; end if;
  if exists(select 1 from public.wall_posts where user_id=v_user_id and mission_id=v_history.challenge_id and created_at>=v_history.completed_at) then raise exception 'This Mission has already been posted'; end if;
  if v_history.proof_type='text' then v_content:=jsonb_build_object('text',left(coalesce(p_proof_text,''),280));
  elsif v_history.proof_type in ('photo','video') then
    if v_history.proof_bucket<>'public-proofs' or v_history.proof_path is null or p_proof_path is distinct from v_history.proof_path then raise exception 'Public media proof is required to post to the Wall'; end if;
    v_content:=jsonb_build_object('path',v_history.proof_path);
  else raise exception 'Unsupported proof type'; end if;
  insert into public.wall_posts(user_id,mission_id,category,difficulty,mission_text,proof_type,proof_content) values(v_user_id,v_history.challenge_id,v_history.category,v_history.difficulty,v_history.challenge_name,v_history.proof_type,v_content) returning * into v_post;
  return jsonb_build_object('id',v_post.id);
end; $$;
grant execute on function public.create_wall_post(uuid,text,text) to authenticated;
